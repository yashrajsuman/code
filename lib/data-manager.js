...
class DataManager {
  ...
  getTopicProgress(userId, subjectId, topicId) {
  ...
  // Session Management
  startLearningSession(userId, topicId) {
    if (!this.isClient()) return {}
  ...
  completeLearningSession(sessionId, xpEarned, coinsEarned, score) {
    if (!this.isClient()) return
  ...
  getLearningSessionsForUser(userId) {
    if (!this.isClient()) return []
  ...
  importUserData(data) {
    ...
    try {
      ...
      if (data.progress) {
        data.progress.forEach((p) => this.saveUserProgress(p))
      }
      return true
    } catch (error) {
